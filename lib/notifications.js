...
async function notify(conn, userId, category, opts = {}) {
	try {
		// Aceita conexão de promise ou de callback. Dizer no comentário que
		// é "uma conexão" não impediu o erro de acontecer: um chamador passou
		// um pool de callback, e o `await conn.execute(...)` abaixo virou um
		// objeto não iterável -- mas a morte não foi essa. Foi o mysql2, mais
		// tarde, chamando um callback que ninguém passou: exceção sem dono,
		// que o try/catch daqui não alcança, derrubando o serviço inteiro.
		//
		// Por isso a normalização mora aqui e não só em quem chama: esta
		// função promete não derrubar ninguém, e essa promessa não pode
		// depender de cada chamador ter acertado o tipo do pool.
		if (conn && typeof conn.promise === "function") conn = conn.promise();
