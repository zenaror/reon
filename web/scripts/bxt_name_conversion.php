...
// Cross-region name / message / region conversion helpers for PokÃ©mon ranking,
// battle tower, and related data.
    ...
    /**
     * Map a source player_region into the destination regionâ€™s supported range.
     *
...
function bxt_name_table_for_region(string $region): ?string
     ...
     * - $useSpecialRules:
     *   - ã£/ãƒƒ -> duplicate first letter of next romaji syllable
     *   - ãƒ¼   -> duplicate last letter of current romaji output
     */
    function bxt_jp_to_en_core(string $jp, array $map, bool $useSpecialRules): string
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if ($useSpecialRules && ($ch === 'ã£' || $ch === 'ãƒƒ')) {
                $romajiNext = '';
                for ($j = $i + 1; $j < $count; $j++) {
                    $next = $chars[$j];
    ...
    /**
     * Rough EN (Aâ€“Z) -> Japanese katakana converter for names.
     */
    /**
     ...
     * - Uses mb_strlen so PokÃ©mon names with multibyte glyphs are counted
...
$kana = implode('', $out);
    ...
    /**
     * Load the default PokÃ©mon name bytes for a given region/species.
     */
...
function bxt_load_default_pokemon_name_bytes(
    ...
    function bxt_is_default_pokemon_name(
        ...
        // same characters (e.g., ã‚«ãƒ“ã‚´ãƒ³ vs default table entry).
        $tableId = null;
    ...
    /**
     * Convert PokÃ©mon nickname for cross-region download, with default-name check.
     *
    ...
    function bxt_convert_pokemon_name_for_download(
        ...
        $safeDefaultForDest = bxt_load_default_pokemon_name_bytes($destRegion, $species);
        if (!is_string($safeDefaultForDest) || $safeDefaultForDest === '') {
            $safeDefaultForDest = str_repeat("\x50", $destMaxBytes);
        }
        if (strlen($safeDefaultForDest) > $destMaxBytes) {
            $safeDefaultForDest = substr($safeDefaultForDest, 0, $destMaxBytes);
        }
        while (strlen($safeDefaultForDest) < $destMaxBytes) {
            $safeDefaultForDest .= "\x50";
        }

        if ($isSourceJ && !$isDestJ) {
            $jpText = bxt_simple_decode_bytes_to_text($nicknameBytes, 'jp');
            if ($jpText === '') {
                return $safeDefaultForDest;
            }
            ...
            $converted = bxt_simple_encode_text_to_bytes($ascii, $tableId, $destMaxBytes);
            if (bxt_trim_name_bytes($converted) === '') {
                return $safeDefaultForDest;
            }
            return $converted;
        }
        ...
        if (!$isSourceJ && $isDestJ) {
            ...
            if ($srcTable === null) {
                $destDefault = bxt_load_default_pokemon_name_bytes('j', $species);
                ...
                return str_repeat("\x50", $destMaxBytes);
            }
        ...
        $safeFallback = $safeDefaultForDest;

        $srcTable  = bxt_name_table_for_region($sourceRegion);
        $destTable = bxt_name_table_for_region($destRegion);
        ...
        $converted = bxt_simple_encode_text_to_bytes($plain, $destTable, $destMaxBytes);
        if (bxt_trim_name_bytes($converted) === '') {
            $normalised = bxt_normalise_latin_for_jp($plain);
            $converted = bxt_simple_encode_text_to_bytes($normalised, $destTable, $destMaxBytes);
        }
    ...
    function bxt_convert_player_name_for_download(
        ...
        $isSourceJ = ($sourceRegion === 'j');
        $isDestJ   = ($destRegion === 'j');
        $safeFallback = str_repeat("\x50", $destMaxBytes);
    ...
    /**
     * Build a fixed-length Easy Chat fallback message for a region.
     */
    function bxt_easy_chat_build_fallback_message(string $region): string
    {
        $r = strtolower($region);
        $phrases = ($r === 'j') ? 6 : 4;
        $code = ($r === 'j') ? 0x0005 : 0x0046;

        $out = '';
        for ($i = 0; $i < $phrases; $i++) {
            // Easy Chat codes in these payloads are little-endian words.
            $out .= pack('v', $code);
        }
        return $out;
    }
    ...
    /**
     * Convert an Easy Chat message to destination-safe width/format.
     *
     * This is conservative at JP boundaries because phrase-level remapping
     * between JP and non-J tables is not available here.
     */
    function bxt_convert_easy_chat_message_for_download(
        string $downloadRegion,
        string $sourceRegion,
        string $bytes
    ): string {
        ...
        $destPhrases = ($downloadRegion === 'j') ? 6 : 4;
        $destLen = $destPhrases * 2;

        if ($downloadRegion === $sourceRegion) {
            $out = substr($bytes, 0, $destLen);
            if (strlen($out) < $destLen) {
                $out = str_pad($out, $destLen, "\x00", STR_PAD_RIGHT);
            }
            return $out;
        }

        // JP -> non-J: use the known-safe non-J fallback and apply existing
        // shortening logic to keep non-J layout constraints.
        if ($sourceRegion === 'j' && $downloadRegion !== 'j') {
            $seed = bxt_easy_chat_build_fallback_message($downloadRegion);
            $out = $seed;
            if (function_exists('bxt_shorten_easy_chat_bytes_for_nonj_message')) {
                $out = bxt_shorten_easy_chat_bytes_for_nonj_message(
                    $seed,
                    $sourceRegion,
                    $downloadRegion
                );
            }
            $out = substr($out, 0, $destLen);
            if (strlen($out) < $destLen) {
                $out = str_pad($out, $destLen, "\x00", STR_PAD_RIGHT);
            }
            return $out;
        ...
        // non-J -> JP: use known-safe JP fallback.
        if ($sourceRegion !== 'j' && $downloadRegion === 'j') {
            return bxt_easy_chat_build_fallback_message($downloadRegion);
        }

        // non-J <-> non-J: keep source message bytes and normalize width.
        $out = substr($bytes, 0, $destLen);
        if (strlen($out) < $destLen) {
            $out = str_pad($out, $destLen, "\x00", STR_PAD_RIGHT);
        }
    ...
    function bxt_transform_ranking_row_for_download(
        ...
        if ($sourceRegion !== $downloadRegion &&
            isset($row['player_region']) &&
        ...
        if ($sourceRegion !== $downloadRegion && isset($row['player_zip'])) {
            $row['player_zip'] = bxt_sanitise_zip_for_cross_region($row['player_zip']);
        }

        if ($sourceRegion !== $downloadRegion &&
            isset($row['player_name']) &&
            is_string($row['player_name']) &&
            function_exists('bxt_convert_player_name_for_download')) {
            $destNameLen = ($downloadRegion === 'j') ? 5 : 7;
            $row['player_name'] = bxt_convert_player_name_for_download(
                $downloadRegion,
                $sourceRegion,
                $row['player_name'],
                $destNameLen
        ...
        if (isset($row['player_message']) && is_string($row['player_message'])) {
            $row['player_message'] = bxt_convert_easy_chat_message_for_download(
                $downloadRegion,
                $sourceRegion,
                $row['player_message']
            );
    ...
    /**
     * Transform a single Battle Tower Pokémon blob for download.
     *
     * Battle Tower mon blobs are:
     * - JP:    54 bytes (48-byte party core + 6-byte mon name field)
     * - non-J: 59 bytes (48-byte party core + 11-byte mon name field)
     */
    function bxt_transform_battle_tower_pokemon_blob_for_download(
        ...
        $destBlobLen   = $isDestJ ? 54 : 59;
        $sourceBlobLen = strlen($blob);
        if ($sourceBlobLen < 49) {
            return $blob;
        }

        $nameOffset    = 48;
        $sourceNameLen = $isSourceJ ? 6 : 11;
        $destNameLen   = $isDestJ ? 6 : 11;

        if ($sourceBlobLen < ($nameOffset + $sourceNameLen)) {
            $blob = str_pad($blob, $nameOffset + $sourceNameLen, "\x50", STR_PAD_RIGHT);
            $sourceBlobLen = strlen($blob);
        }

        $sourceNameBytes = substr($blob, $nameOffset, $sourceNameLen);
        $species = ord($blob[0]);

        $destNameBytes = $sourceNameBytes;
        if (function_exists('bxt_convert_pokemon_name_for_download')) {
            $destNameBytes = bxt_convert_pokemon_name_for_download(
                $downloadRegion,
                $sourceRegion,
                $species,
                $sourceNameBytes,
        ...
        if (!is_string($destNameBytes)) {
            $destNameBytes = '';
        }
        $destNameBytes = substr($destNameBytes, 0, $destNameLen);
        if (strlen($destNameBytes) < $destNameLen) {
            $destNameBytes = str_pad($destNameBytes, $destNameLen, "\x50", STR_PAD_RIGHT);
        }

        $out = substr($blob, 0, $destBlobLen);
        if (strlen($out) < $destBlobLen) {
            $out = str_pad($out, $destBlobLen, "\x00", STR_PAD_RIGHT);
        }

        $out = substr_replace($out, str_repeat("\x50", $destNameLen), $nameOffset, $destNameLen);
        $out = substr_replace($out, $destNameBytes, $nameOffset, $destNameLen);
    ...
    /**
     * Transform a Battle Tower trainer row for download.
     *
     * Applies conversion for every language pair so cross-language non-J rows
     * also use destination-safe name and message bytes.
     */
    function bxt_transform_battle_tower_trainer_for_download(
        ...
        if (isset($row['name']) && is_string($row['name']) &&
            function_exists('bxt_convert_player_name_for_download')) {
            $destNameLen = ($downloadRegion === 'j') ? 5 : 10;
        ...
        foreach (['message_start', 'message_win', 'message_lose'] as $msgKey) {
            if (isset($row[$msgKey]) && is_string($row[$msgKey])) {
                $row[$msgKey] = bxt_convert_easy_chat_message_for_download(
                    $downloadRegion,
                    $sourceRegion,
                    $row[$msgKey]
                );
